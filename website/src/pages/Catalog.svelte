<script>
  ...
  let query = "";
  let filter = "all";
  const matchesQuery = (effect) => `${effect.name} ${effect.group} ${effect.summary}`.toLowerCase().includes(query.toLowerCase());
  $: motions = motionDocs.filter((effect) => (filter === "all" || effect.kind === filter) && matchesQuery(effect));
  $: components = componentDocs.filter((effect) => (filter === "all" || filter === "component") && matchesQuery(effect));
  $: visibleCount = motions.length + components.length;
</script>
...
<main class="catalog container">
  ...
  <div class="toolbar">
    ...
    <span class="result-count">{String(visibleCount).padStart(2, "0")} / {effectDocs.length}</span>
  </div>
  ...
  {#if !visibleCount}<p class="empty">No motion matches that search.</p>{/if}
</main>
